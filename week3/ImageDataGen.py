...
class NumpyArrayIterator(Iterator):
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        #print("channels_axis ----------------------------- ",channels_axis)
...
class DirectoryIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        #mean_=np.load('mean.npy')
        #std_=np.load('std.npy')
        #file = open('depths.csv')
        #depth = {}
        #for i,line in enumerate(file):
        #    if i>0:
        #        id,z=  line.split(',')
        #        depth[id] = float(z.replace('\n',''))

        #mean = np.mean(list(depth.values()))
        #std = np.std(list(depth.values()))
        #depth_norm = { id: (z-mean)/std for id,z in depth.items()}
        #list_name=[]
        #file_name=[]
        for i, j in enumerate(index_array):
            fname = self.filenames[j]
            #print(fname)
            #list_name.append(depth_norm[fname.split('/')[1].split('.')[0]])
            #file_name.append(fname.split('/')[1].split('.')[0])
        ...
        if self.class_mode == 'input':
            ...
            if self.image_data_generator.ismask:
                #print(batch_x.shape,batch_x.min(),batch_x.max())
                #print("--------=========-------- ",batch_x) #(2, 360, 480, 1)
                temp = np.round(batch_x).reshape((batch_x.shape[0],256,1600)).astype("int32") #(2, 360, 480,1)
                #print("self.image_data_generator.nClasses:", self.image_data_generator.nClasses)
                cat = to_categorical(temp, self.image_data_generator.nClasses)
                return cat
            if self.image_data_generator.isnorm:
                return {'img':(batch_x-mean_)/std_ , 'z': np.array(list_name)}
            if self.image_data_generator.name_file:
                return  {'img':batch_x , 'z': np.array(list_name),'Name': np.array(file_name) }  
            #print(X_new.shape,X_new.min(),X_new.max())
            X_new = batch_x / 127.5 - 1.
            return X_new
        return batch_x, batch_y
